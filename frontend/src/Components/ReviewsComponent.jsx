...
const ReviewsComponent = ({ businessName }) => {
    ...
    const fetchReviews = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/localreviews/${encodeURIComponent(businessName)}`
        );
  ...
  const handleDeleteReview = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8000/api/localreviews/${id}`);
    ...
    try {
      if (editingReview) {
        ...
        const response = await axios.patch(
          `http://localhost:8000/api/localreviews/${editingReview._id}`,
          {
            ...reviewData,
        ...
        // Add new review
        const response = await axios.post('http://localhost:8000/api/localreviews', {
          ...reviewData,
          businessName,
      ...
      calculateAverageRating(editingReview ? 
        reviews.map(review => review._id === editingReview._id ? {...review, ...reviewData} : review) :
        [...reviews, {...reviewData, businessName, userName: user.username}]
      );
